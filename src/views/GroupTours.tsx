...
const GROUP_TYPES = [
  ['Adventure Groups', 'Waterfalls, hiking, camping, Knuckles adventures, Ella trails, rafting and off-the-beaten-path experiences.'],
  ['Wildlife & Safari Groups', 'Leopard safaris, birdwatching, elephant encounters, whale watching and photography-focused journeys.'],
  ['Wellness & Retreat Groups', 'Yoga retreats, Ayurveda, meditation programs, wellness escapes and slow-travel concepts.'],
  ['Golf Tours', 'Premium golf experiences across Colombo, Digana, Nuwara Eliya and Hambantota.'],
  ['Photography & Nature Tours', 'Sunrise trails, cultural and wildlife photography, waterfalls, tea estates and hidden landscapes.'],
  ['Cultural & Heritage Tours', 'Temples, ancient kingdoms, UNESCO sites, village life, local traditions and authentic cuisine.'],
  ['Corporate & Incentive Travel', 'Team outings, MICE programs, incentive tours, executive retreats and company adventures.'],
  ['Private Social Groups', 'Family reunions, birthdays, friend circles, honeymoon groups and customized celebrations.'],
] as const;
...
export function GroupTours() {
  return <>
    ...
    <section className="bg-sand py-20 lg:py-28"><div className="mx-auto max-w-content px-6"><SectionHeading eyebrow="Tour Styles" title="Groups we welcome" subtitle="Choose a theme or combine several interests in one custom journey."/><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{GROUP_TYPES.map(([title,copy],i)=><Reveal key={title} delay={(i%4)*.04} className="h-full"><article className="h-full rounded-xl border border-jungle/10 bg-ivory p-7 shadow-card"><UsersIcon className="h-7 w-7 text-gold-dark" strokeWidth={1.4}/><h3 className="mt-5 font-serif text-xl text-jungle">{title}</h3><p className="mt-3 text-sm leading-7 text-jungle-muted">{copy}</p></article></Reveal>)}</div></div></section>
